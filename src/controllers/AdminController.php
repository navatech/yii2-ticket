...
class AdminController extends Controller {
	 ...
	 * Проверяем если $mailSendAnswer === true значит делаем отправку уведомления од ответе пользователю создавшему
	 * тикет
	 *
	 * @param $id int
	 *
	...
	public function actionClosed($id) {
		...
		if ($this->module->mailSend !== false) {
			(new Mailer())->sendMailDataTicket($model->topic, $model->status, $model->id, '')->setDataFrom(Yii::$app->params['adminEmail'], $this->module->subjectAnswer)->senda('closed');
		}
	...
	public function actionOpen() {
		...
		return $this->render('open', [
			...
			'departments' => $this->module->departments,
			'users'       => $users,
		]);
	}
}