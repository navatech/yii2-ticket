...
class Mailer extends \yii\swiftmailer\Mailer {
	...
	public  $viewPath = '@navatech/ticket/views/ticket/mail';

	private $nameTicket;

	/** @var string Название тикета */
	private $textTicket;

	/** @var string Тест сообщения */
	private $status;

	/** @var integer Статус тикета */
	private $subject;

	/** @var string Тема email сообщения */
	private $urlTicket;

	/** @var  string url тикета на который был дан ответ */
	private $setTo;

	/** @var string email юзера которому будет отправлено сообщение */
	...
	public function senda($view) {
		$this->mail->compose($view, [
			...
			'status'     => $this->status,
			'link'       => $this->urlTicket,
		])->setFrom(\Yii::$app->params['adminEmail'])->setTo($this->setTo)->setSubject($this->subject)->send();
	}
